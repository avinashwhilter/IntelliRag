...
swagger_template = {
    ...
    "info": {
        "title": "RAGFlow API",
        "description": "Swagger UI for RAGFlow backend",
        "version": "1.0.0",
    },
    "securityDefinitions": {
        "ApiKeyAuth": {"type": "apiKey", "name": "Authorization", "in": "header"},
...
# ✅ Fix: Ensure /flasgger_static actually serves the right JS/CSS
FLASGGER_STATIC_PATH = Path(sys.prefix) / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages" / "flasgger" / "static"

if FLASGGER_STATIC_PATH.exists():
    @app.route("/flasgger_static/<path:filename>")
    def flasgger_static(filename):
        return send_from_directory(str(FLASGGER_STATIC_PATH), filename)
